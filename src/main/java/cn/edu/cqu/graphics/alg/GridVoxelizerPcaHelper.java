...
public class GridVoxelizerPcaHelper {
    /**
    ...
    private Map<Integer, Long> pointIndexMap = new HashMap<>();
     ...
     */
    public void voxelize(List<Point3d> points, double cellSize) {
        this.points = points;
    ...
    public List<GridCell> fetchCells(List<Integer> indices) {
        Set<Long> set = new HashSet<>();
        for (int index : indices) set.add(pointIndexMap.get(index));
        List<GridCell> cells = new ArrayList<>();
        for (Long cellIndex : set) cells.add(cellMap.get(cellIndex));
        return cells;
    }

    private void determineVoxelNumber() {
        BoundingBox tightBox = BoundingBox.of(points);
        double averageLength = cellSize;
